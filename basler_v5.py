...
class BaslerCamera(CameraBase):
    ...
    def set_mode(self, mode: str) -> None:
        ...
        prev = self._mode
        super().set_mode(mode)
        if prev == mode and self._camera is not None:
            # Re-apply anyway in case the camera lost the setting
            self._apply_mode_settings()
            return
        self._apply_mode_settings()
    ...
    def _apply_mode_settings(self):
        ...
        Basler nodes can silently reject writes while grabbing — stop first,
        ...
        try:
            if was_grabbing:
                try:
                    ...
                    print(f"[Basler] StopGrabbing failed: {e}")
            ...
            if self._mode == "normal":
                ...
                for value in ("Continuous", "Once"):
                    try:
                        ...
                        continue
                if not wb_ok:
                    print("[Basler] BalanceWhiteAuto not supported on this camera")
                # Try a reasonable gamma for display
                try:
                    self._camera.Gamma.SetValue(1.0)
                except Exception:
                    pass
            else:
                # Raw mode: turn off auto white balance, but leave the
                # camera's factory BalanceRatio values alone. The user
                # explicitly wants the unprocessed sensor look.
                try:
            ...
            if was_grabbing:
                try:
                    ...
                    print(f"[Basler] StartGrabbing failed: {e}")
    ...
    def grab_raw(self) -> Optional[np.ndarray]:
        """
        Sensor-native frame.
        For 12-bit formats, MSB-align the data into uint16 so saved TIFFs
        display correctly in Windows Photos and other viewers. Without this,
        12-bit values (0-4095) fill only 6% of the 16-bit range and the
        image looks black. Precision is unchanged - the low 4 bits are zero.
        """
