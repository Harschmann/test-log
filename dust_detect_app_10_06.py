"""
Dust Detection App  (synced zoom/pan + texture rejection)
---------------------------------------------------------
Workflow:
  1. Load Image
  2. Type a radius, then CLICK each camera center on the left panel.
     (each click uses the current radius; change it for different cameras)
  3. Undo Last / Clear All to fix mistakes.
  4. Detect -> result on the right.
  5. Save Result -> writes before|after image to the results folder.
  6. Open Folder -> opens that folder.

Image panel controls:
  - Mouse wheel  = zoom (both panels sync)
  - Click + drag = pan (both panels sync)
  - Quick click on LEFT panel = add a camera center

Detection (two passes):
  - Bright pass: plain threshold -> big/bright dust.
  - Faint pass : tophat -> faint dust, with TWO texture defenses:
        * Median blur removes TV-noise / salt-pepper grain.
        * Texture density gate: a faint spot is rejected if its
          surrounding window is crowded with other bright pixels
          (texture is DENSE; real dust is ISOLATED). Self-excluded so
          a large genuine spot does not trigger itself.

Tuning the texture gate:
  - "Tex max%"  : lower = stricter (rejects more texture). 100 = off.
  - "Tex window": size of the neighborhood checked around each spot.
...
def run_detection(img, centers, p):
    ...
    # denoise: median crushes TV-noise / salt-pepper grain (odd size only)
    if p["median"] >= 3 and p["median"] % 2 == 1:
        blurred = cv2.medianBlur(blurred, p["median"])
    ...
    half = max(2, p["tex_win"] // 2)
    use_tex_gate = p["tex_max"] < 100
    ...
    for c in cont2:
        ...
        (x, y), r = cv2.minEnclosingCircle(c)
        xi, yi = int(x), int(y)
        if big_detected[yi, xi] == 255:
            continue

        # texture density gate: reject spots sitting in a crowded (textured) area
        if use_tex_gate:
            x0, x1 = max(0, xi - half), min(bf.shape[1], xi + half)
            y0, y1 = max(0, yi - half), min(bf.shape[0], yi + half)
            win = bf[y0:y1, x0:x1].copy()
            cv2.circle(win, (xi - x0, yi - y0), int(r) + 3, 0, -1)  # erase self
            if win.size > 0 and (win > 0).mean() * 100.0 > p["tex_max"]:
                continue

        cv2.circle(result, (xi, yi), max(10, int(r) + 4), (0, 0, 255), 3)
        count += 1
...
class DustApp:
    def __init__(self, root):
        ...
        self.median_var = tk.StringVar(value="3")
        self.texwin_var = tk.StringVar(value="81")
        self.texmax_var = tk.StringVar(value="6")
    ...
    def _build_ui(self):
        ...
        ttk.Separator(c2, orient="vertical").pack(side=tk.LEFT, fill="y", padx=6)
        ...
        ttk.Separator(c2, orient="vertical").pack(side=tk.LEFT, fill="y", padx=6)
        self._setting(c2, "Median", self.median_var)
        self._setting(c2, "Tex window", self.texwin_var)
        self._setting(c2, "Tex max%", self.texmax_var)
    ...
    def _params(self):
        return {
            ...
            "median": self._read_int(self.median_var, 3),
            "tex_win": self._read_int(self.texwin_var, 81),
            "tex_max": self._read_float(self.texmax_var, 6.0),
        }
