...
class MainWindow(ctk.CTk):
    ...
    def _build_left_sidebar(self):
        ...
        ctk.CTkLabel(self.left, text="").pack(expand=True)
        ctk.CTkButton(
    ...
    def _build_right_sidebar(self):
        ...
        self.mode_switch = ctk.CTkSwitch(
            self.right, text="Raw Mode (sensor data)",
        ...
        ctk.CTkLabel(
            self.right,
            text=("• Normal: 8-bit BGR TIFF\n"
                  "    (white-balanced, debayered)\n"
                  "• Raw: 12-bit Bayer TIFF\n"
                  "    (sensor-native, lossless)"),
        ...
        ctk.CTkLabel(self.right, text="").pack(expand=True)
        ctk.CTkLabel(
            self.right,
    ...
    def _auto_connect(self):
        ...
        self.camera = cam
        # Apply the current UI mode to the freshly connected camera
        try:
            self.camera.set_mode(self.mode)
        except Exception as e:
            print(f"set_mode on connect failed: {e}")
    ...
    def _capture_one(self):
        ...
        if self.mode == "raw":
            ...
            # Use the processed BGR frame from the live preview (already
            # white-balanced via the camera's pipeline)
        ...
        if ok:
            ...
            kind_label = "RAW" if self.mode == "raw" else "Image"
    ...
    def _toggle_mode(self):
        ...
        # Reconfigure the camera (white balance, gamma, etc.)
        if self.camera is not None:
            try:
                self.camera.set_mode(self.mode)
            except Exception as e:
