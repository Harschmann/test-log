    ...
    def _apply_mode_settings(self):
        ...
        try:
            ...
            if self._mode == "normal":
                ...
                # Raw mode: no color processing at all
                try:
                    self._camera.BalanceWhiteAuto.SetValue("Off")
                except Exception:
                    pass
                # Reset balance ratios so we get true sensor values
                try:
                    for ch in ("Red", "Green", "Blue"):
                        try:
                            self._camera.BalanceRatioSelector.SetValue(ch)
                            self._camera.BalanceRatio.SetValue(1.0)
                        except Exception:
                            pass
                except Exception:
                    pass
